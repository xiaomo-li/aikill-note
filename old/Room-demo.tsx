...
> {
  ...
  private playerName: string =
    this.props.electronLoader.getData(ElectronData.PlayerName) || "unknown";
  ...
  @mobx.observable.ref
  private defaultMainVolume = this.props.electronLoader.getData(
    ...
    : 50;
  @mobx.observable.ref
  private defaultGameVolume = this.props.electronLoader.getData(
  ...
  constructor(
    ...
    this.roomId = Math.random() * 100000;
    ...
    this.connectionService = this.props.getConnectionService(
      this.roomMode === RoomMode.Campaign
    );

    if (this.roomMode === RoomMode.Campaign) {
      this.socket = new LocalClientEmitter(
    ...
    this.baseService = installService(
  ...
  private readonly createWaitingRoomCaller = (
    ...
    this.props.history.push(`/waiting-room/${roomId}`, {
      ...
      hostConfig: this.props.config.host.find(
        (host) => host.hostTag === this.gameHostedServer
      ),
    });
  };

  private readonly onHandleBulkEvents = async (
  ...
  private animation<T extends GameEventIdentifiers>(
    identifier: T,
    event: ServerEventFinder<T>
  ) {
    this.baseService.Animation.GuideLineAnimation.animate(identifier, event);
    this.baseService.Animation.MoveInstantCardAnimation.animate(
      identifier,
      event
    );
  }
    ...
    if (
      ...
      for (const info of (
        ...
        if (
          unengagedMessage &&
          engagedPlayerIds &&
          !engagedPlayerIds.includes(this.store.clientPlayerId)
        ) {
  ...
  @mobx.action
  private readonly onSwitchSideBoard = () =>
    (this.renderSideBoard = !this.renderSideBoard);
          ...
          <div className={styles.roomBoard}>
            ...
            <Dashboard
              ...
              outsideCardEnableMatcher={
                this.store.clientPlayerOutsideCardActionsMatcher
              }
